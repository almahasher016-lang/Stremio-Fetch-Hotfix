...
# Version bump.
for p in ['package.json','package-lock.json','src/release.js','README.md']:
    s=r(p)
    s=s.replace('4.4.1','4.4.2')
    w(p,s)
...
# Regression test at pure availability level. We intentionally avoid network preflight by candidates without fetchable URLs.
