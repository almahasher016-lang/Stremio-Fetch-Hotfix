...
test('partial provider results cannot erase a richer timing-compatible candidate pool', async () => {
  ...
  const previous = [
    {
      id: 'bluray-good', provider: 'opensubtitles', lang: 'ara', releaseName: 'Show.S01E07.720p.BluRay.x264-BLOODY', score: 850,
      releaseMatch: releaseMatch({ tier: 2, priority: 19000, mismatched: ['quality', 'releaseGroup'] }), scoreReasons: [],
    },
    {
      id: 'web-old', provider: 'opensubtitles', lang: 'ara', releaseName: 'Show.S01E07.2160p.WEB-DL-GRP', score: 1400,
      releaseMatch: releaseMatch({ tier: 2, priority: 21000, mismatched: ['source'] }), scoreReasons: [],
    },
    {
      id: 'web-alt', provider: 'subdl', lang: 'ara', releaseName: 'Show.S01E07.1080p.WEBRip-GRP2', score: 900,
      releaseMatch: releaseMatch({ tier: 1, priority: 12000, mismatched: ['source', 'quality'] }), scoreReasons: [],
    },
  ];
  const degradedFresh = [{
    id: 'web-partial', provider: 'opensubtitles', lang: 'ara', releaseName: 'Show.S01E07.2160p.WEB-DL-PARTIAL', score: 1800,
...
test('a newly discovered exact hash still outranks preserved older candidates', async () => {
  ...
  const previous = [{
    id: 'old-family', provider: 'subdl', lang: 'ara', releaseName: 'Movie.2026.1080p.BluRay-GRP', score: 1500,
    releaseMatch: releaseMatch({ tier: 4, priority: 43000 }), scoreReasons: [],
  }];
  const fresh = [{
    id: 'new-hash', movieHash: 'abcd1234', provider: 'opensubtitles', lang: 'ara', releaseName: 'Movie.2026.1080p.WEB-DL-OTHER', score: 400,
