...
export function buildConfig(env = process.env) {
  ...
  return {
    ...
    ranking: {
      ...
      enableAutoSyncOption: experimentalSyncAllowed && toBool(get('ENABLE_AUTO_SYNC_OPTION'), false),
      autoSyncMinConfidence: toInt(get('AUTO_SYNC_MIN_CONFIDENCE'), 70, 0, 100),
      enableReferenceAutoSync: experimentalSyncAllowed && toBool(get('ENABLE_REFERENCE_AUTO_SYNC'), false),
      referenceLanguage: get('REFERENCE_SYNC_LANGUAGE', 'en'),
      strictReleaseMatching: toBool(get('STRICT_RELEASE_MATCHING'), false),
      minRankScore: toInt(get('MIN_RANK_SCORE'), 180, -1000, 3000),
      maxStremioSubtitles: toInt(get('STREMIO_MAX_SUBTITLES'), 12, 1, 20),
      maxReferenceOptions: toInt(get('STREMIO_REFERENCE_TOP'), 1, 0, 10),
      maxAutoSyncOptions: toInt(get('STREMIO_AUTOSYNC_TOP'), 1, 0, 10),
      maxOriginalOptions: toInt(get('STREMIO_ORIGINAL_TOP'), 10, 0, 20),
    },
    resolver: {
    ...
    timingEvidence: {
      ...
      safeAutoAlign: toBool(get('TIMING_EVIDENCE_SAFE_AUTO_ALIGN'), true),
      safeMinConfidence: toInt(get('TIMING_EVIDENCE_SAFE_MIN_CONFIDENCE'), 94, 0, 100),
      safeMinAgreement: toNumber(get('TIMING_EVIDENCE_SAFE_MIN_AGREEMENT'), 0.82, 0, 1),
      safeMinCoverage: toNumber(get('TIMING_EVIDENCE_SAFE_MIN_COVERAGE'), 0.72, 0, 1),
      safeMinCueRatio: toNumber(get('TIMING_EVIDENCE_SAFE_MIN_CUE_RATIO'), 0.68, 0, 1),
      safeMaxResidualMedianMs: toInt(get('TIMING_EVIDENCE_SAFE_MAX_MEDIAN_RESIDUAL_MS'), 1500, 100, 10000),
      safeMaxResidualP90Ms: toInt(get('TIMING_EVIDENCE_SAFE_MAX_P90_RESIDUAL_MS'), 4000, 500, 30000),
      safeMaxOffsetMs: toInt(get('TIMING_EVIDENCE_SAFE_MAX_OFFSET_MS'), 60000, 500, 300000),
      safeMinRatio: toNumber(get('TIMING_EVIDENCE_SAFE_MIN_RATIO'), 0.97, 0.90, 1.10),
      safeMaxRatio: toNumber(get('TIMING_EVIDENCE_SAFE_MAX_RATIO'), 1.03, 0.90, 1.10),
    },
    slo: {
    ...
    referenceSync: {
      enabled: experimentalSyncAllowed && toBool(get('ENABLE_REFERENCE_AUTO_SYNC'), false),
      minConfidence: toInt(get('REFERENCE_SYNC_MIN_CONFIDENCE'), 88, 0, 100),
      minCues: toInt(get('REFERENCE_SYNC_MIN_CUES'), 24, 2, 100),
      minCueRatio: toNumber(get('REFERENCE_SYNC_MIN_CUE_RATIO'), 0.75, 0, 1),
      ...
      piecewise: toBool(get('REFERENCE_SYNC_PIECEWISE'), false),
      minReferenceMatchScore: toInt(get('REFERENCE_SYNC_MIN_REFERENCE_MATCH_SCORE'), 420, 0, 5000),
      minAnchorCoverage: toNumber(get('REFERENCE_SYNC_MIN_ANCHOR_COVERAGE'), 0.70, 0, 1),
      minTemporalAgreement: toNumber(get('REFERENCE_SYNC_MIN_TEMPORAL_AGREEMENT'), 0.82, 0, 1),
      dtwEnabled: toBool(get('REFERENCE_SYNC_DTW_ENABLED'), false),
    ...
    encodingProxy: {
      ...
      maxFallbacks: toInt(get('ENCODING_PROXY_MAX_FALLBACKS'), 4, 0, 8),
